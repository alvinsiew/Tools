...
from dnn_network.smallervggnet import SmallerVGGNet
...
Y_pred = model.predict_generator(aug.flow(testX, testY, batch_size=BS), steps = len(testX) // BS, verbose=1)

print(Y_pred.shape)
print(testY.shape)

minLen = 0
if Y_pred.shape[0] >= testY.shape[0]:
	minLen = testY.shape[0]
else:
	minLen = Y_pred.shape[0]
